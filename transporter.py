...
COLOR_LOST_TIMEOUT = 5    # timeout gdy nie widzimy danego koloru (s)

# Parametry obrotu (jeżeli wykryjemy kolor jednym sensorem -> obracamy aż drugi zobaczy)
ROTATE_SPEED = 35           # procentowy speed przy obracaniu (dla koła jadącego do przodu)
ROTATE_TIMEOUT = 1.5        # maks czas obracania aby wyrównać drugi czujnik (s)
...
def normalized_color_name(sensor):
    """Zwraca jednolita nazwę koloru z sensora (np. 'blue','red','green','black','white','yellow','brown') lub None."""
    try:
        code = sensor.color
        mapping = {
            sensor.COLOR_BLACK: 'black',
            sensor.COLOR_BLUE: 'blue',
            sensor.COLOR_GREEN: 'green',
            sensor.COLOR_YELLOW: 'yellow',
            sensor.COLOR_RED: 'red',
            sensor.COLOR_WHITE: 'white',
            sensor.COLOR_BROWN: 'brown'
        }
        ...
        try:
            ...
            if name is None:
                return None
            name = name.lower()
            for cname in ['blue','red','green','black','white','yellow','brown']:
                if cname in name:
                    return cname
            return None
        except Exception:
...
def rotate_until_both_see(target_color, initial_side, timeout=ROTATE_TIMEOUT):
    ...
    while True:
        ...
        if l == target_color and r == target_color:
            success = True
            break
...
def rotate_180_degrees(speed_pct=30, fallback_seconds=1.5):
    ...
    try:
        robot.turn_degrees(180, SpeedPercent(speed_pct))
        print(">>> Obrót 180° (turn_degrees) zakończony.")
    except Exception as e:
        print(">>> Brak turn_degrees lub błąd (fallback). Exception:", e)
        try:
            robot.on(SpeedPercent(-speed_pct), SpeedPercent(speed_pct), brake=True)
            sleep(fallback_seconds)
        finally:
            robot.off()
        print(">>> Obrót 180° (fallback czasowy) zakończony.")
...
def state_follow_black(now, L, R, left_name, right_name):
    ...
    if now - last_check_time >= CHECK_INTERVAL:
        ...
        if seen_color_now is not None and seen_color_now not in seen_colors:
            ...
            if initial_side is not None:
                print(f">>> Nowy kolor '{seen_color_now}' wykryty przez {initial_side}. Przechodzę do ROTATING.")
    ...
    if not is_rotating:
        robot.on_for_seconds(SpeedPercent(leftcirclespeed), SpeedPercent(ricghtcirclespeed), 0.01, brake=False, block=False)
...
def state_rotating(now, L, R, left_name, right_name):
    ...
    print(f">>> ROTATING: próbuję wyrównać kolor '{target}' od strony {initial_side}.")
    ...
    if success:
        ...
        print(f">>> ROTATING zakończone sukcesem — oba sensory widzą '{target}'. Przechodzę do FOLLOW_COLOR.")
        return 'FOLLOW_COLOR'
    else:
...
def state_follow_color(now, L, R, left_name, right_name):
    ...
    if left_name == 'black' or right_name == 'black':
        ...
        ricghtcirclespeed = BASE_SPEED
        color_seen_time = None
        print(f">>> Wykryto CZARNY podczas śledzenia '{prev}'. Wracam do FOLLOW_BLACK.")
        return 'FOLLOW_BLACK'
    ...
    if color_seen_time is not None and (now - color_seen_time) > COLOR_LOST_TIMEOUT:
        ...
        ricghtcirclespeed = BASE_SPEED
        color_seen_time = None
        print(f">>> UTRACONO KOLOR '{prev}', wracam do FOLLOW_BLACK.")
        return 'FOLLOW_BLACK'
...
def state_action(now, L, R, left_name, right_name):
    ...
    robot.off()
    print(f">>> ACTION: oba sensory widzą '{left_name}'. Stan chwytaka = '{grip_state}'")
...
try:
    ...
    while True:
        ...
        current_gain = COLOR_ERROR_GAINS.get(current_target, 1.0)
        print("[T:{}] L_ref={}, R_ref={}, L_col={}, R_col={}, error={:.2f}, gain={:.2f}, speeds L={}, R={}, seen={}, grip={}, state={}".format(
            current_target.upper(), L, R, left_name, right_name, error, current_gain, leftcirclespeed, ricghtcirclespeed, seen_colors, grip_state, current_state
        ))
