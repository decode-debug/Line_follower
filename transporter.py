...
ROTATE_SPEED = 10
# maks czas obracania aby wyrównać drugi czujnik (s)
...
def read_both_colors():
    ...
    return normalized_color_name(left_color_sensor), normalized_color_name(right_color_sensor)  # noqa: E501


def choose_detected_color(left_name, right_name, prefer_list):
    """
    Jeśli któryś z sensorów widzi kolor z prefer_list zwróć go według priorytetu prefer_list.  # noqa: E501
...
def apply_pid_speed_control(current_left_speed, current_right_speed, error_value, gain=1.0):  # noqa: E501
    """Wspólna logika PID + aktualizacji prędkości, zwraca (new_left, new_right, correction)."""  # noqa: E501
...
def rotate_until_both_see(target_color, initial_side, timeout=ROTATE_TIMEOUT):
    """
    Obraca robota w stronę initial_side ('left' lub 'right') aż obydwa czujniki zobaczą target_color  # noqa: E501
    lub upłynie timeout. Zwraca True jeśli oba czujniki widzą kolor, False jeśli timeout.             # noqa: E501
    ...
    # kierunek obrotu: gdy left widzi -> obracamy w lewo (left wstecz, right do przodu)  # noqa: E501
    ...
    while True:
        ...
        if (l == target_color and initial_side == 'right') or (r == target_color and initial_side == 'left'):  # noqa: E501
            success = True
            break
...
def backup_backward(speed_pct=BACKUP_SPEED, seconds=BACKUP_SECONDS):
    ...
    print(">>> Cofam do tyłu przez {:.2f}s z prędkością {}%.".format(
        seconds, speed_pct))
    try:
        robot.on_for_seconds(SpeedPercent(-speed_pct),
                             SpeedPercent(-speed_pct), seconds, brake=True, block=True)  # noqa: E501
    except Exception:
...
def state_follow_black(now, L, R, left_name, right_name):
    ...
    Jeśli wykryjemy nowy kolor z preferencji -> ustawiamy rotate_target i przechodzimy do ROTATING.  # noqa: E501
    W tym stanie także wykonujemy normalne PID-owe sterowanie kół.
    """
    global current_target, seen_colors, last_check_time, leftcirclespeed, ricghtcirclespeed, pid, color_seen_time  # noqa: E501
    ...
    if now - last_check_time >= CHECK_INTERVAL:
        ...
        if seen_color_now is not None and seen_color_now not in seen_colors:
            ...
            if initial_side is not None:
                print(">>> Nowy kolor {} wykryty przez {}. Przechodzę do ROTATING.".format(  # noqa: E501
...
def state_rotating(now, L, R, left_name, right_name):
    ...
    Jeżeli uda się wyrównać -> FOLLOW_COLOR, w przeciwnym razie -> FOLLOW_BLACK.  # noqa: E501
    """
    global current_target, seen_colors, leftcirclespeed, ricghtcirclespeed, pid

    # zapamiętaj target (może zostać zmieniony w międzyczasie)
    target = current_target
    # ustal początkową stronę (jeżeli mamy - jeśli nie -> próbuj obrócić domyślnie w prawo)  # noqa: E501
    initial_side = None
    ...
    success = rotate_until_both_see(
        target, initial_side, timeout=ROTATE_TIMEOUT)
    if success:
        # udało się — dodaj do seen, ustaw kolor jako aktywny i przejdź do FOLLOW_COLOR  # noqa: E501
        ...
        print(">>> ROTATING zakończone sukcesem — oba sensory widzą '{}'. Przechodzę do FOLLOW_COLOR.".format(target))  # noqa: E501
        return 'FOLLOW_COLOR'
    else:
...
def state_follow_color(now, L, R, left_name, right_name):
    ...
    """
    global current_target, color_seen_time, leftcirclespeed, ricghtcirclespeed, pid, last_action_time  # noqa: E501
    ...
    if left_name == 'black' or right_name == 'black':
        ...
        ricghtcirclespeed = BASE_SPEED
        color_seen_time = None
        print(">>> Wykryto CZARNY podczas śledzenia '{}'. Wracam do FOLLOW_BLACK.".format(prev))  # noqa: E501
        return 'FOLLOW_BLACK'
    ...
    if color_seen_time is not None and (now - color_seen_time) > COLOR_LOST_TIMEOUT:  # noqa: E501
        prev = current_target
        current_target = 'black'
        pid.reset()
        leftcirclespeed = BASE_SPEED
    ...
    if (not is_rotating) and (left_name is not None) and (left_name == right_name) and (left_name == current_target):  # noqa: E501
        if now - last_action_time >= ACTION_COOLDOWN:
            return 'ACTION'
...
def state_action(now, L, R, left_name, right_name):
    ...
    Po akcji przechodzimy z powrotem do FOLLOW_COLOR (lub FOLLOW_BLACK jeśli występuje).  # noqa: E501
    """
    global grip_state, last_action_time, leftcirclespeed, ricghtcirclespeed, pid, color_seen_time  # noqa: E501
...
>>>>>>> 154d17ec48a8a5723a563cc236af2fffa1776eb5
        ...
        if next_state != current_state:
            print(">>> STATE TRANSITION: {} -> {}".format(current_state, next_state))  # noqa: E501
